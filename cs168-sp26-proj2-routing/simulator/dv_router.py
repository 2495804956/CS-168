...
class DVRouter(DVRouterBase):
    ...
    def handle_data_packet(self, packet, in_port):
        ...
        if packet.dst in self.table:
            ...
            # if the latency along the outgoing link is greater than or equal to INFINITY,drop the packet.
            if entry.latency >= INFINITY:
                return
            self.send(packet, port=entry.port)
        # If no route exists for the packet’s destination,drop the packet.(do noting)

        ##### End Stage 2 #####

    def send_routes(self, force=True, single_port=None):
        """
        ...
        for port in ports_to_send:
            ...
            for dst, entry in self.table.items():
                ...
                # stage 8:Rule 7(Count to infinity)
                if latency >= INFINITY:
                    latency = INFINITY

                # stage 6:Rule 6A(Split Horizon)
                if self.SPLIT_HORIZON and entry.port == port:
                    continue

                # stage 7:Rule 6B(Poison Reverse)
    ...
    def expire_routes(self):
        ...
        for dst, entry in list(self.table.items()):
            if entry.expire_time <= api.current_time():
                ...
                # stage 9:Rule 5(Poisoning Expired Routes)
                if self.POISON_EXPIRED:
                    self.table[dst] = TableEntry(dst=dst, port=entry.port, latency=INFINITY, expire_time=api.current_time()+self.ROUTE_TTL)
                else:
                    self.table.pop(dst)
                    self.log("Removing expired route to %s",dst)
    ...
    def handle_route_advertisement(self, route_dst, route_latency, port):
        ...
        if current_entry is None or new_latency < current_entry.latency:
            update = True
