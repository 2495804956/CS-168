    ...
    for ttl in range(1, TRACEROUTE_MAX_TTL+1):
        ...
        for attempt in range(PROBE_ATTEMPT_COUNT):
            ...
            if recvsock.recv_select():  # Check if there's a packet to process.
                ...
                icmp_header = ICMP(buf[icmp_start:])

                if icmp_header.type == 11 or icmp_header.type == 3:
                    router_ip = ip_header.src
                    if router_ip not in find_ips:
                        find_ips.append(router_ip)
        ...
        util.print_result(find_ips, ttl)
        path.append(list(find_ips))
