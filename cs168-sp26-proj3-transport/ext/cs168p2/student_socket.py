...
class StudentUSocket(StudentUSocketBase):
  ...
  def close(self):
    ...
    if self.state is CLOSED:
      ...
      ## Start of Stage 7.1 ##
      self.fin_ctrl.set_pending(next_state=FIN_WAIT_1)   # Set the intention to send a FIN and transition to FIN_WAIT_1 after sending
      ...
      ## Start of Stage 6.2 ##
      self.fin_ctrl.set_pending(next_state=LAST_ACK)   # Set the intention to send a FIN and transition to LAST_ACK after sending
  ...
  def connect(self, ip, port):
    ...
    ## Start of Stage 1.1 ##
    self.snd.nxt = self.snd.iss   # 设置下个发送序列号为初始发送序列号
    p = self.new_packet(ack=False, syn=True)  # 创建一个新的TCP包，设置SYN标志
    self.tx(p)  # 发送SYN包
    self.state = SYN_SENT  # 将状态设置为SYN_SENT
    self.snd.nxt = self.snd.nxt |PLUS| 1              # 更新下一个序列号
  ...
  def tx(self, p, retxed=False):
    ...
    if (p.tcp.SYN or p.tcp.FIN or p.tcp.payload) and not retxed:
      ...
      ## End of Stage 4.4 ##
      p.tx_ts = self.stack.now  # record the time when the packet is sent
      self.retx_queue.push(p)  # add the packet to the retransmission queue
      pass
  ...
  def handle_synsent (self, seg):
    ...
    if acceptable_ack:
      ...
      if self.snd.una |GT| self.snd.iss:
        # 收到的SYN-ACK确认了我们发送的SYN，连接建立成功
      
        self.snd.nxt = seg.ack     # 将下一个序列号设置为 ACK 数据包的 ACK 编号
  ...
  def update_rto(self, acked_pkt):
    ...
    ## Start of Stage 9.1 ##
    self.R = self.stack.now - acked_pkt.tx_ts  # 计算往返时间 R
    if self.R is None:
      self.srtt = self.R
    ...
    # Clamp the RTO to be within the specified bounds
  ...
  def handle_accepted_payload(self, payload):
    ...
    self.set_pending_ack()  # We need to ACK this data
  ...
  def handle_accepted_fin(self, seg):
    ...
    ## Start of Stage 7.2 ##
    elif self.state in FIN_WAIT_1:
      if seg.ack :
        # if receive a FIN-ACK packet
        self.set_pending_ack()
  ...
  def check_ack(self, seg):
    ...
    if self.state in (ESTABLISHED, FIN_WAIT_1, FIN_WAIT_2, CLOSE_WAIT, CLOSING):
      ## Start of Stage 4.1 ##
      # If the ack number represents a byte you haven’t sent yet, drop the packet.
      if seg.ack |GT| snd.nxt:
        continue_after_ack = False
        return
      # If the ack number of the received segment represents a sent but unacked packet, call self.handle_accepted_ack on the segment.
      elif seg.ack |GE| snd.una:
        self.handle_accepted_ack(seg)
      # If the ack number had already previously been acked before,drop the segment
      else:
        continue_after_ack = False
    ...
    if self.state == FIN_WAIT_1:
      ...
      # delete the connection state if the ACK acknowledges our FIN
      if self.fin_ctrl.acks_our_fin(seg.ack):
        self._delete_tcb()
  ...
  def maybe_send(self):
    ...
    while remaining > 0 and self.tx_data:
      playload_size = min(len(self.tx_data), self.mss, remaining)
      
      playload = self.tx_data[:playload_size]
      p = self.new_packet(data=playload)
      self.tx(p)
      self.tx_data = self.tx_data[playload_size:]

      num_pkts += 1
      bytes_sent += len(playload)
      remaining = (snd.una |PLUS| snd.wnd) |MINUS| snd.nxt
